...
void obj_flagpole::addmodel(ctilepos ctp, chunkmesh& cmesh)
{
    //tiledata::addblock(ctp, 2, 0, 255, 255, notint, 0, 0, notint, cmesh);
    map_obj_models::addmodel(cmesh, ctp, 2, forwardside);

    if (flagtextureid >= 0)
        map_obj_models::addmodel(cmesh, ctp, 3, forwardside, flagtextureid);
}
