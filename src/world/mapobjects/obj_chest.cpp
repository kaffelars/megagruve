...
void obj_chest::addmodel(ctilepos ctp, chunkmesh& cmesh, chunklightcontainer& sunlight, chunklightcontainer& tilelight)
{
    //tiledata::addblock(ctp, 2, 0, 255, 255, notint, 0, 0, notint, cmesh);
    if (lidopen)
        map_obj_models::addmodel(cmesh, sunlight, tilelight, ctp, 1, forwardside);
    else
        map_obj_models::addmodel(cmesh, sunlight, tilelight, ctp, 0, forwardside);
}
